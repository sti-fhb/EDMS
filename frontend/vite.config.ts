...
export default defineConfig({
  ...
  test: {
    ...
    //
    // | 條件 | 結果 | 牆鐘 | environment |
    // |---|---|---|---|
    // | 預設 ~12 worker | **19 檔紅 / 35 條紅** | 487s | 2199s |
    // | `maxWorkers:4`（機器忙）| 74/74 全綠 | 500s | 863s |
    // | `maxWorkers:4`（機器忙，更忙）| **69/74——5 支 worker 起不來** | 1198s | 1581s |
    // | **`maxWorkers:4`（機器空閒）** | **74/74 全綠** | **268s** | 398s |
    //
    // ⭐ 空閒時 **268s vs 487s，快 1.8 倍**，`environment` 少 5.5 倍——本設定**同時**
    // 改善可靠性與速度。
    //
    // 🔴 前三列不可拿來比較：這台機器由多個 session 共用，第二、三列都被別人的負載污染。
    // 本註解初版曾據第二列寫成「牆鐘持平（+2.7%）、不要期待變快」，那是錯的——**拿一個
    // 被污染的量測當基準**。任何效能量測前先 `ps -W | grep -c node.exe` 確認機器空閒。
    //
    // ⚠️ `maxWorkers` 是**每個 vitest 行程**的上限，不是機器層級的。三個 session 各跑一次
    // 就是 3 × 4 = 12 個並行 fork，又回到原點（第三列即如此）。多 session 並行時的紅
    // **不代表本設定失效**。
    //
