...
此指令預期只發布文件類異動（`docs/` 底下）。當偵測到 `docs/` 以外的異動（`backend/`、`frontend/`、`e2e/`、`nginx/`、`scripts/`、`tools/` 等），**不直接納入、也不直接擋掉**，而是：

1. 逐一列出這些檔案，並判斷並說明其作用類型（例如：後端程式、前端程式、E2E 測試、資料庫 migration、部署／設定檔等），必要時附一句該檔異動的簡短說明。
2. 明確告知使用者「這些不是文件異動，正常應走程式發布流程（如 `/sti-implement`、`/sti-commit`）」。
3. 詢問使用者是否仍要把這些檔案一併放進本次 PR。使用者未明確確認前，一律不加入。
...
- 選擇 `1`：`git add` 所有異動的 `docs/` 檔案（排除非 docs 的異動）
  ...
  git add docs/
  ```
  若有 `docs/` 以外的異動，依「發布範圍提醒」處理：逐一列出並說明每個檔案的作用類型（後端程式／前端程式／E2E 測試／資料庫 migration／部署設定等），告知這些非文件異動正常應走程式發布流程，再詢問使用者是否仍要一併加入。使用者明確確認後才 `git add {檔案}`。
