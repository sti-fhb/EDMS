...
- **SA Q 留言**：含標記 `<!-- sti-plan:sa-questions -->` 的留言。找到 → 比對其後是否有 SA 的回覆留言：
  ...
  - SA 尚未回覆 → **讀每題的 `阻塞類型` 欄位**（`/sti-plan` 已結構化標註），依類型分級提示後**照常繼續，不阻斷**（是否該等 SA 由使用者人工判斷），不要壓平成單一句：
    - 有 `backend-schema` 未回 → `⚠️ Q{n} 卡後端 schema / API 介面，建議等 SA 回覆再動後端骨架。`
    - 有 `cross-module` 未回 → `⚠️ Q{n} 卡跨模組依賴，先確認是否影響 critical path。`
    - 未回的僅為 `none` → `ℹ️ N 條 SA Q 未回覆但不卡任何骨架，可直接開工。`
    - 未回的含 `frontend-ux`（無 backend-schema / cross-module）→ `ℹ️ Q{n} 純前端 UX、不卡後端，可先做 Phase 1 後端；前端 Phase 2 待 SA。`
    - 舊格式無 `阻塞類型` 欄位（相容）→ `⚠️ 偵測到 N 條 SA Q 尚未回覆，請確認是否該等 SA 回覆再開工。`
...
**若無 PLAN_COMMENT**：先套 `/sti-common`「複雜度分級（預判 issue 規模）」評估本 issue——

- 若**達複雜門檻**（任一指標為複雜）→ 提示使用者（不強制，維持流程選做）：
  ```
  ⚠️ 本 issue 達複雜門檻但無 /sti-plan 規劃留言。建議先跑 /sti-plan #{Issue 編號} 產出完整規劃與 SA Q，再回來 /sti-implement。
    1. 仍直接開工（我自行從頭規劃）
    2. 先去跑 /sti-plan（結束本次 implement）
  ```
  - 選 `1` → 依下列來源從頭產出計畫（原流程）
  - 選 `2` → 顯示提示後結束本次 implement
- 若為**簡單**（未達門檻）→ 直接依下列來源從頭產出計畫（原流程）。
...
### 步驟 13：Commit & PR（委派給 /sti-commit + /sti-pr-create）

⚠️ **本步驟不自行實作 commit / push / PR 邏輯**，一律**委派**給專責指令，讓安全檢查（掃 `.env` / 機密 / `console.log`）與 PR 模板只有**單一事實來源**——見 `/sti-commit` 步驟 2、`/sti-pr-create` 步驟 3。implement 只負責把 Issue 編號、變更摘要、CI 狀態等上下文帶過去，不再平行重寫（避免內建版與專責指令 drift、出現「用 implement 送 code 反而漏檢查」）。

先顯示 `git status` 讓使用者確認檔案清單（Worktree 模式在 `{WORKTREE}` 下執行，Branch 模式在專案根目錄執行）。

詢問使用者：
```
請選擇下一步：
  1. commit → push → 建立 Draft PR（一氣呵成）
  2. 僅 commit → push（稍後再建 PR）
  3. 結束（稍後自行執行 /sti-commit + /sti-pr-create）
```
...
- **選擇 2** → 只委派 **`/sti-commit`**（commit + push），完成後提示：
  ```
  ✅ 已 commit & push。稍後建立 PR 請執行：/sti-pr-create #{Issue 編號}
  ```
- **選擇 3** → 顯示提示後結束：
  ```
  下一步：
  1. 執行 /sti-commit 提交變更
  2. 執行 /sti-pr-create #{Issue 編號} 建立 PR
  3. PR 合併後執行 /sti-cleanup #{Issue 編號} 清理分支
     （worktree 模式：請先回到主 repo 目錄再執行）
  ```

> Draft PR 的「不觸發 CI、不輪詢、不指派 Reviewer」與「轉正時機」一律依 `/sti-pr-create` 與 `/sti-pr-ready` 既有規範，本步驟不重述。
