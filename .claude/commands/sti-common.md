...
以下指令使用此規則：`/sti-sa-precheck`、`/sti-plan`

命中任一即可直接跳過盤點 / 自檢（`/sti-plan` 改用 `/sti-implement` 直接開工）：

- 純 bug fix（根因明確、改動範圍小且明確；SD 約 < 100 行，SA 視規格改動局部可控）
- 純樣式調整（CSS / 文字）
- 跟版 / 套件升級
- 單純 refactor，不改外部行為

判斷句：「這個 issue 開工前我需要對齊什麼東西嗎？」答「沒有」就跳過。
