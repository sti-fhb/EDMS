...
執行前請讀取以下規則檔確認 TBMS Python 規範：
- `.claude/rules/sti-backend-modules.md` — AppError、OperatorInfo、AuditLogService、paginate()、BaseModel
- `.claude/rules/sti-backend-logging.md` — 禁止 print()、敏感資料 log 規範
- `.claude/rules/sti-testing.md` — pytest fixture、Auth fixture、覆蓋率規則
- `.claude/rules/sti-error-codes.md` — error_code 命名與新增規範
- `.claude/rules/sti-alembic-rules.md` — Alembic migration 規範
