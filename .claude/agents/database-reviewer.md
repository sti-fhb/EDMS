...
a) Row Level Security
   - RLS enabled where tenant isolation needed?
   - RLS policies use (SELECT current_setting('app.current_user_id')) pattern?
   - RLS columns indexed?
...
- 硬刪除（`DELETE FROM`）而非軟刪除（`DELETED = 1`）
...
執行前請讀取以下規則檔確認 TBMS 資料庫規範：
- `.claude/rules/sti-alembic-rules.md` — Migration 撰寫規範、DDL 操作、Seed Data
- `.claude/rules/sti-backend-modules.md` — BaseModel 共用欄位、paginate() helper
- `.claude/rules/sti-naming-conventions.md` — DB table/column 命名規則（UPPER_SNAKE_CASE）
