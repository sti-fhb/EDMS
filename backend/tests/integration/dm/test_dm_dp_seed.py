...
驗證 DM 之 9 通知範本（MODULE=DM）與 DM_ 參數已種入 DP 表，且可經 SRVDP001（ParamService）
讀回——確認 TBMS 前例 A（各模組 migration 種進 DP 共用表、前綴 / MODULE 隔離）落地。
"""
...
async def test_dm_templates_seeded_in_dp_table(db):
    """9 個 MODULE=DM 通知範本種入 DP_NOTIFY_TEMPLATE。"""
    ...
    assert {"DOC_SUBMIT", "DOC_PUBLISH", "KPI_WEEKLY", "UNREAD_REMIND", "AUTO_REMIND"} <= codes
