...
class TestAccessGate:
    ...
    async def test_具_et_角色者放行並帶出角色集(self, db) -> None:
    ...
    async def test_停用之角色不計入(self, db) -> None:
    ...
    async def test_多重角色取聯集(self, db) -> None:
...
class TestModuleGateRegistration:
    ...
    async def test_四閘皆已註冊(self, db) -> None:
        register_et_module()  # 冪等，測試可重入
    ...
    async def test_is_module_admin_僅對管理者為真(self, db) -> None:
        register_et_module()
    ...
    async def test_has_any_role_對任一角色為真(self, db) -> None:
        register_et_module()
...
class TestDefaultRoleProvisioning:
    ...
    async def test_經聚合閘呼叫亦生效(self, db) -> None:
        register_et_module()
