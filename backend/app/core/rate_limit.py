...
#
# ⚠️ 代價：event loop 被 bcrypt 打飽所需的 IP 數自 58（門檻 5）降至約 10（門檻 30）。
# hash_password 為同步呼叫、實測 209ms，且部署要求 --workers 1。放寬 IP 門檻後，
# 可用性的實際防線是「bcrypt 移出 event loop + 全域配額」——見 #214，該 issue 因此
# 從「建議」升為必要。
