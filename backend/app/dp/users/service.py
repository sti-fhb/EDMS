...
_PWD_EXPIRY_TEMPLATE = "PWD_EXPIRY_REMIND"
...
class UsersService:
    ...
    async def disable_idle_accounts(self, db: AsyncSession) -> int:
        ...
        從未登入者以 CREATED_DATE 為基準（見 repository.find_idle_active）。逐筆 savepoint 容錯：
        單一帳號失敗不擋其餘。回傳成功禁用筆數；不 commit（由呼叫方交易負責）。
        """
            ...
            try:
                async with db.begin_nested():
                    await self._repo.set_status(
                        db, user=user, status="DISABLED", operator_id=_SYSTEM_USER, now=now
                    )
    ...
    async def send_pwd_expiry_reminders(self, db: AsyncSession) -> int:
        ...
        每日跑均寄（不去重，spec_us11 FR-05）；逐筆容錯。回傳寄出筆數；不 commit（由呼叫方交易負責）。
        """
