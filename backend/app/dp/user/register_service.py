...
class RegisterService:
    ...
    async def register(
        ...
        # 2. Email 唯一（快取路徑；並發競態由 step 4 的 IntegrityError 兜底）
        ...
        # email_exists 與此處寫入間有 TOCTOU 空窗，並發同 Email 由 DB UNIQUE 兜底 →
        # 攔 IntegrityError 轉為乾淨的 409，避免競態落到通用 500。
        now = utcnow()
        ...
        try:
            ...
            raise AppError(status_code=409, detail=_EMAIL_TAKEN_MSG, error_code="DP_USER_001") from exc
        # 5. 首筆密碼歷程（SEQ_NO=1）
