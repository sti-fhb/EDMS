...
def build_item_state(
    ...
    發布檢核有 `BLOCK_QUIZ_NO_QUESTION`，但那是**發布當下**的一次性檢核。

    🔴 **2026-09-23（#410）起，上游已補上守門**：`quiz/service._ensure_not_last_question_of_live_course`
    不許把「已發布且未視同關閉」課程的測驗刪到 0 題。所以本例外**已退化為安全網**
    ——它保護的是**該守門上線前**就已經是 0 題的既有資料，而不是持續被製造的新狀態。

    ⛔ **不要因此把本例外拿掉。** 兩層方向相反、互補：上游讓新資料進不了這個狀態，
    下游讓既有的 0 題測驗不鎖死學員。只留上游的話，守門上線前既存的 0 題測驗仍會把
    整門課後半段永久鎖死，而畫面上完全看不出原因。
    ...
    ⚠️ **上面舉的那個情境，已發布課程走不到了**（#410 的守門擋住「刪到 0 題」）——
    但**這條 ⛔ 仍然成立**，別據此認為它過時。它保護的不是那個情境，是**守門上線前
    就已經是 0 題的既有資料**：那些資料按定義沒經過守門，而擋住它們一樣會讓整門課
    後半段永久鎖死。⛔ 判準是「這個狀態**存不存在**」，不是「它**還會不會被製造**」。
    ...
    參與完課或核可判定，本例外立刻變成偽造完訓紀錄的路徑**——屆時上游守門（#410 已建）
    擋得住新資料，但**擋不住守門上線前就存在的 0 題測驗**，故那種改動仍須連同本例外
    一起重新評估，不能只靠上游。
