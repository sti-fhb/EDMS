...
1. **60 秒有效**——access log 裡的票在被讀到時幾乎必然已過期
...
代價是一個**最長 60 秒的窗口**——學員在發票後被移除、或帳號被停用，仍可播完當下這
一支影片。這在內部訓練系統可接受：他本來就已經看過那些內容，而下一支影片會因發票
端點的授權失敗而拿不到票。
"""
...
#: 票的有效秒數。夠 `<video>` 發出第一個請求，短到 log 裡撿到也沒用。
#:
#: ⚠️ 不要為了「影片很長」而調大——票只用於**發起**連線，之後的 Range 請求由瀏覽器
#: 沿用同一個 URL，而串流本身不會因票過期而中斷（連線已建立）。
