...
### 3. 密碼運算的容量取決於 CPU 與連線池（#214）

bcrypt 單次約 **185 ms**（本機實測），跑在專用執行緒池（`min(4, 可用 CPU)` 條）。併發閘的
門檻**由連線池容量推導**——因為能進到閘的請求都已握著一條 DB 連線（`get_db` 的 session
持有到請求結束），門檻若不低於連線池就永遠不會觸發：

```
_MAX_INFLIGHT   = max(workers, (DB_POOL_SIZE + DB_MAX_OVERFLOW) // 2)  夾在 容量-1 以下
_HASH_MAX_INFLIGHT = _VERIFY_MAX_INFLIGHT = 75% × _MAX_INFLIGHT
```

預設（pool 5 + overflow 10 = 15、4 個 worker）→ 總門檻 **7**、雜湊類與認證類各 **5**。
容量 < 4 時閘會退化（門檻等於容量、或分艙保留歸零），**啟動即擋**。

尖峰參考數字（M/M/4/5 損失模型，40 人自助註冊）：

| 40 人送出的時間跨度 | 約有多少人收到 503 |
|---|---|
| 30 秒（各自填表的正常節奏，實測間隔約 18 秒/人） | ≈ 0 |
| 10 秒 | ≈ 0 |
| 5 秒 | ≈ 1 |
| 1~2 秒（例如講師喊「一起按送出」） | **7~12** |

⚠️ 若確定要支援「整班同時送出」，**唯一能擴大緩衝的旋鈕是提高 `DB_MAX_OVERFLOW`**
（門檻隨連線池容量上升；例如 overflow 調到 30 → 容量 35 → 總門檻 17、雜湊類 12）。
調整前須確認 PostgreSQL 的 `max_connections` 容得下，且該池為全 app 共用（DM / ET / 排程）。
另注意被卸除的請求**已先消耗註冊端點的限流配額**（`hit` 在 dependency 階段），故 503 之後
重試可能改撞 429；此互動已知，未於 #214 處理。
