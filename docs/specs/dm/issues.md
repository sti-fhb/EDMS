...
**階段**：收尾
**涵蓋 Tasks**：T060（P1 生命週期 e2e）、T061（簽核分支 + 單一送審週期約束）、T062（跨模組 SRVDM001/002 + ET 取最新版）、T063（權限職責分離）、T063a（標籤式可見性 SC-010）、T063b（閱讀 KPI / 排程 SC-011）、T064（永久保留 append-only）、T065（func_name 並發唯一）、T066（安全性檢查）、T067（效能 SC-001）

## 任務說明

DM 全 US（US1/US3~US13）已交付，本 issue 為**收尾**：補齊**跨 US 端到端整合測試**（單一 US 測試涵蓋不到的完整生命週期與模組交界）、彙整**權限 / 可見性 / 永久保留 / 並發唯一**之驗證，並執行**安全性檢查（T066）與效能驗證（T067）**，確認 spec.md 之 SC-001 / SC-010 / SC-011 達標。

> ℹ️ **與各 US 既有測試的關係**：各 US 交付時已附自身整合測試（如 US3 可見性、US13 KPI/排程、US6 簽核催辦、US8 廢止、US10/11 唯讀查詢）。本 issue **不重跑**各 US 已覆蓋者，聚焦：**(1)** 跨 US 串接的 e2e 流程（T060~T062）、**(2)** 以「驗證清單」形式彙整並補齊職責分離 / 可見性 / 永久保留 / 並發唯一之缺口（T063~T065）、**(3)** 專項安全審查（T066）與效能量測（T067）。既有覆蓋處於本 issue 以「引用既有測試 + 補缺」處理。

## 範圍

**整合測試（跨 US 端到端）**
- **T060 P1 生命週期 e2e**：單一測試串接 新增（US5）→ 送審（US5）→ 核准發布（US6）→ 文件庫檢索（US3）→ 詳細頁（US4）→ 編輯新版本（US5）→ 簽核（US6）→ 廢止申請（US8）→ 核准廢止（US8），驗各階段狀態機與資料一致。
- **T061 簽核分支 + 單一送審週期約束**：退回（DRAFT + `DM_DOC_012`）/ 撤回送審（WITHDRAWN）/ 自動催辦（SCHDM002）；同一文件不可兩種送審並存。
- **T062 跨模組**：`SRVDM001` / `SRVDM002`（ET 引用 DM 教材，US12）+ DM 發布新版後 ET 取最新版（無快取延遲）。
...
**安全 / 效能**
- **T066 安全性檢查**：SSO / JWT 認證邊界、檔案上傳大小 / 類型限制、密碼雜湊、個資（Email / 姓名）處理；以 `security-reviewer` 專項審查 + 必要補測。
- **T067 效能**：文件庫檢索（多條件 + 標籤 AND + 分頁）回應時間 **P95 ≤ 2 秒**（SC-001）——以量測腳本 / 測試驗證並記錄。

## 驗收條件

- [ ] T060 P1 生命週期 e2e 測試綠（新增→…→核准廢止全串接）
- [ ] T061 簽核分支（退回 / 撤回 / 催辦）+ 單一送審週期約束驗證綠
- [ ] T062 跨模組 SRVDM001/002 + ET 取最新版（無快取延遲）驗證綠
- [ ] T063 職責分離（排除自審 / 角色聯集 / admin-only URL / 分區可見）驗證綠
- [ ] T063a 可見性（SC-010）驗證綠：閱覽者過濾正確、編輯/審核/管理見全部、停用不收回、後端防繞過
- [ ] T063b 閱讀 KPI / 排程（SC-011）確認達標（引用 US13 既有測試 + 補缺）
- [ ] T064 永久保留：軟刪除 / append-only 不可竄改驗證綠
- [ ] T065 func_name 並發唯一 + 友善訊息驗證綠
- [ ] T066 安全性檢查完成（security-reviewer 無 Critical / High；發現項修正或記錄）
- [ ] T067 檢索 P95 ≤ 2 秒（SC-001）量測達標並記錄
- [ ] `uv run pytest -q` 全綠；前端測試通過；ruff / ESLint / type-check / 覆蓋率門檻通過

## 依賴

- **全部 DM US 已交付**：US1（#1）/ US3（#2）/ US4（#3）/ US5（#4）/ US6（#5）/ US7（#6）/ US8（#7）/ US9（#8）/ US10（#9）/ US11（#10）/ US12（#11）/ US13（#12）——本 issue 為其上之收尾驗證，不新增功能。
- **#127 Foundation（已交付）**：共用件（授權 / DOC_ID / 檔案服務 / 通知接線 / 狀態機 / catalog / 可見性）。
- 平台 DP：SSO / JWT、`DP_EMAIL_LOG` outbox、`DP_SCHEDULE` 引擎、`DP_USER`——皆既有。

## 注意事項

- **不重複造輪**：各 US 既有整合測試為既有覆蓋，本 issue 引用 + 補跨 US 缺口，避免重寫。
- **可見性維持現行設計**：T063a 驗「編輯 / 審核 / 管理者見全部」——#253（編輯者可見對象收斂）已決議 **not planned**、維持 spec_us3 FR-008，不在本 issue 收斂。
- **e2e 資料來源**：以種子 / fixture 或串接真實 API 產生已發布文件；跨 US 流程測試須落在 integration 層（真 DB）。
- **T066 安全**：以 `security-reviewer` agent 專項審查為主，聚焦 lint 抓不到的認證邊界 / 輸入流向 / 個資處理；發現 CRITICAL 立即修。
- **T067 效能**：P95 量測環境與資料量需記錄（避免空庫假達標）；SC-001 為驗收門檻。
- **既有 follow-up**（非本 issue 阻塞，可另處理）：US6 補寫 `DM_CHANGE_LOG.NOTE`（發布事件備註）、dp/audit CSV sanitizer 收斂至 `core/csv_export`。

## 相關文件

- [spec.md](spec.md)（SC-001 / SC-010 / SC-011、Success Criteria）、[tasks.md](tasks.md)（T060~T067）、[data-model.md](data-model.md)（append-only 表、func_name 部分唯一索引）
- 各 US 規格：[spec_us3.md](spec_us3.md)（可見性）、[spec_us6.md](spec_us6.md)（簽核 / outbox）、[spec_us13.md](spec_us13.md)（KPI / 排程）等
