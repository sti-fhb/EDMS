...
- **Decision**: **每版本單一檔案**；檔案實體存於檔案系統 / 物件儲存，DB（`DM_DOC_VERSION`）僅存檔名 / 路徑 / 大小 / MIME 等 metadata，**不存 BLOB**。單檔上限預設 50 MB（由平台 `DP_PARAM.DM_FILE_MAX_MB`〔前綴 `DM_`〕控制）。
...
- **與 ET 對齊、無「部門」資料源（2026-07-02）**: DM 的 `DM_TAG(AUDIENCE)` / `DM_USER_TAG` 對應 ET 的 `ET_TAG（受訓單位標籤）` / `ET_USER_TAG`，種子值一致（全體 / 護理師 / 軍人 / 醫檢師 / 行政人員）、皆管理者維護；惟兩系統**各自持有、不共用**（使用者群相交而不相同，per ET 2026-07-02 決策）。權限管理原「部門」欄**無外部資料來源**（DM 不介接主系統 HR / 組織資料），移除；使用者所屬「單位」改由管理者於平台 DP 後台權限管理以「可見對象/單位」授權設定（＝ `DM_USER_TAG`）。
...
> **2026-07-08 集中化**：本節（§9/§9b/§9c/§9d）之通知範本、寄件佇列、發信引擎、系統參數、排程已集中於平台 DP——範本存 `DP_NOTIFY_TEMPLATE`（`MODULE=DM`）、寄送改呼叫平台唯一發信服務並經 outbox `DP_EMAIL_LOG`、參數存 `DP_PARAM`（前綴 `DM_`）、排程於 `DP_SCHEDULE` 註冊由平台引擎執行（`DP_SCHEDULE_LOG` 記錄、job handler 由 DM 提供）；發信引擎調校參數改為平台級 `DP_`。以下決策理由不變，僅落地位置改述。

- **Decision**: 工作流通知經**平台發信服務（SMTP）+ 站內訊息**；**自動催辦僅站內訊息**。催辦以**每日排程作業**掃描 `DM_REVIEW` 中停留 ≥ 催辦門檻天數（平台 `DP_PARAM.DM_REMIND_THRESHOLD`，前綴 `DM_`，預設 7、可調 1–30）之 PENDING_* 項目，每日發一次直到處理完。
...
- **非同步 outbox**: 核准發布交易僅將收件人（快照）交平台唯一發信服務（傳 `template_code`），由平台 outbox `DP_EMAIL_LOG` 批次寄送、標記狀態可重試。理由：一次發布（尤其「全體」）可能通知上千人，同步寄信會拖慢甚至回滾核准交易；outbox 使寄送與交易解耦、失敗不遺漏。
...
- **排程 SCHDM001（每週執行，星期＋時間可設定，預設週一 10:00）**: 一個作業算 KPI→寄管理者週報（Email，內文摘要 + CSV）→寄未看閱覽者提醒（Email，一人一信彙整）；寄信經平台發信服務 + outbox `DP_EMAIL_LOG` 非同步，不阻塞排程；`SCHDM001` 於平台 `DP_SCHEDULE` 註冊、平台引擎執行、`DP_SCHEDULE_LOG` 記錄，job handler 由 DM 提供。新增 `KPI_WEEKLY` / `UNREAD_REMIND` 兩事件（EMAIL_ONLY，存 `DP_NOTIFY_TEMPLATE` MODULE=DM）。**每週執行時間**由管理者於平台 DP 後台「**排程管理**」編輯 `SCHDM001` 的 `CRON_EXPR`（預設 `0 10 * * 0` 即週一 10:00 UTC，即時生效），KPI 週報與未讀提醒共用同一時間同批執行。
...
- **不存內容快照**（2026-06-29，per SA 決定）：平台 outbox `DP_EMAIL_LOG` 僅存 `template_code` + 收件人（+ 發布通知之 DOC_ID），平台發信服務 **寄送時即時組信**（未讀提醒即時算該人未看清單、KPI 週報即時算統計 + CSV）。取捨：寄送時點與排程時點之未看數字可能微幅差異，可接受（更即時）；組信所需業務資料由平台 job / 發信服務反向 import DM service 取得。

## §9d 批量寄信之韌性（outbox 重試 / 限流 / 告警，2026-06-29）

> **2026-07-09 平台對齊（DP plan 定案）**：④ 失敗告警**作廢**——平台 DP 釐清（第 2 輪）不做系統內告警通報，FAILED 率由 IT 以 log / 監控工具追蹤。參數命名對齊平台 `MAIL` 參數組（平台級無前綴：`MAIL.RETRY_MAX`=5 / `MAIL.RATE_PER_MIN`=60 / `MAIL.RETRY_INTERVAL_MIN`=2），原 `DP_MAIL_*` 扁平命名不再使用；②指數退避改為固定重試間隔參數（平台 research §8）。①③ 之韌性精神不變。

- **Decision**: 平台 outbox `DP_EMAIL_LOG` 之發信引擎採：① **最大重試 `DP_MAIL_MAX_RETRY`（預設 5）**，超過標 `FAILED` 不再自動重試；② **指數退避**（約 1 / 2 / 4 / 8 / 16 分）；③ **批次限流 `DP_MAIL_RATE_PER_MIN`（預設 60 封/分，依 SMTP 主機上限調）**；④ **失敗告警 `DP_MAIL_FAIL_ALERT_PCT`（預設 20%）**——單次批次完成後 FAILED 比率超標即通知管理者 / IT。此類調校參數因發信引擎集中於平台，屬**平台級 `DP_` 參數**、由平台管理員維護（不再掛 `DM_`）。
