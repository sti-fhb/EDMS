...
- **Decision**: 登入端點支援 `redirect`（來源模組路徑）參數：有值且屬白名單（ET / DM 路徑）→ 登入成功後前端導回；無值 → 前端路由至模組入口頁。入口頁載入時呼叫「我的模組角色摘要」端點（DP 聚合 ET / DM `is_module_admin` / 有無任一角色）決定 DM 入口顯示與否。
- **Rationale**: spec 釐清第 1 / 2 輪（Q2 / Q4）：入口頁 ET 恆顯、DM 依角色、不顯後台入口；redirect 白名單防 open redirect。
