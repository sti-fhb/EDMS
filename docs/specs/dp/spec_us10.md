...
4. **Given** 查無符合紀錄，**When** 查詢完成，**Then** 顯示空狀態提示（DP-MSG-DP09-001）
...
7. **Given** 非管理者之一般使用者，**When** 嘗試存取本頁或查詢 API，**Then** 伺服器端拒絕（DP-MSG-DP09-002）
...
|---------|------|---------|---------------|
| DP-MSG-DP09-001 | 提示 | 查無符合條件之紀錄 | FR-DP-US10-02 空結果 |
| DP-MSG-DP09-002 | 錯誤 | 無權限存取操作記錄 | FR-DP-US10-01 越權 |
