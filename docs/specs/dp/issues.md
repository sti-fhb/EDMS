...
- ⚠️ **帳號列舉 vs 明確訊息（待 /sti-plan 或 /sti-sa-precheck 確認）**：spec FR-02 明訂「區分帳號不存在 / 密碼錯誤」（LOGIN-001 vs 002），但這與「防帳號列舉」的資安慣例衝突（現有 `DP_AUTH_001` 為合併訊息「帳號或密碼錯誤」）。開工前須 SA 確認採「明確訊息（依 spec）」或「合併訊息（防列舉）」，並據以定 error code。
- **error code**：`DP_AUTH_001`（帳密錯誤，Foundation 已登記）、`DP_AUTH_004`（停用）、`DP_AUTH_005`（鎖定）、`COMMON_429`（速率）已就緒；若採明確訊息需新增「帳號不存在」等碼（依上點裁示）。強制變更為回應旗標、非 error。
