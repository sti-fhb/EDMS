...
- `SELF_REGISTER`（US2 自助註冊）：**不帶密碼**（`PWD_HASH` 為 NULL，#212），使用者於驗證連結**自設密碼**才建 `DP_USER`。
- `ADMIN_INVITE`（US4 管理者邀請）：**不帶密碼**（`PWD_HASH` 為 NULL），使用者於啟用連結**自設密碼**才建 `DP_USER`。

> 兩種 KIND 自 #212 起形狀一致：本表**匿名可寫**（自助註冊為公開端點），密碼素材不得存於此——原設計把註冊當下填的密碼存進本表，使任何人可用他人 Email 註冊並填自己的密碼（帳號預先劫持）。`PWD_HASH` 因此已無任何寫入非 NULL 的路徑（欄位保留待 #215 評估移除）。

一 Email 一筆待驗證（EMAIL UNIQUE），重新申請 / 重寄以 Email 覆蓋；consume 後硬刪除。逾期列由每日排程 SCHDP001 清理，**逾期滿 1 天才刪**（留一天供當天的客服追查，#226）。使用者管理頁「待啟用邀請」頁籤只撈 `KIND=ADMIN_INVITE`——故一筆殘留的 `SELF_REGISTER` 逾期列在 UI 上完全看不見，這是 #226 要修的另一半（管理者代建同一 Email 原本恆回誤導的 409）。

> **「逾期＝視為不存在」為跨讀取點的一致規則**（#212 / #226）：登入（回「請先註冊」而非「請重新寄送」）、匿名重寄（不換發新 token，否則 TTL 形同無效）、管理者代建（照常邀請並清掉死列）三處採同一定義。新增讀取本表的路徑時必須沿用。
...
`ET_VIDEO_ALLOWED_FORMATS`、`ET_VIDEO_MAX_SIZE_MB`、`ET_URGENT_REMIND_DAYS`、`ET_TRAINING_UNIT`（LIST，含 `ALL`）；`DM_DOC_CATEGORY`（LIST，`DETAIL_LOCK`）、`DM_FUNC_NAME`（LIST）、`DM_AUDIENCE`（LIST，含 `ALL`）、`DM_TAG_MODULE` / `DM_TAG_NATURE` / `DM_TAG_LEGAL`（LIST）、`DM_REMIND_THRESHOLD`、`DM_FILE_MAX_MB` / `DM_FILE_TYPES`、`DM_WEEKLY_SCHED_DAY_TIME`。
