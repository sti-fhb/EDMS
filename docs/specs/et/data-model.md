...
| 3 | 測驗說明 | DESCRIPTION | TEXT | N | **純文字**（非 HTML、不經消毒）；顯示於學員作答開始前（2026-08-25 新增）|
| 4 | 及格分數 | PASS_SCORE | INT | Y | 0–100，預設 80 |
| 5 | 作答時間限制（分鐘）| TIME_LIMIT_MIN | INT | N | **空白（NULL）= 不限時**；**≥ 1 = 限時 N 分鐘**（倒數歸零自動提交）；預設空白 |
| 6 | 重考次數上限 | MAX_RETRY | INT | Y | 0–999；**0 = 不允許重考**（僅可作答 1 次）；預設 3 |
| 7 | 版本號 | VERSION | INT | Y | 樂觀鎖 |
| - | 標準欄位 | — | — | — | （同上）|

**業務規則**:
- DESCRIPTION 為純文字欄位，前端以多行文字方塊輸入、以純文字渲染（**不得** `dangerouslySetInnerHTML`）；與 ET_MATERIAL.DESCRIPTION_HTML 之 HTML 消毒路徑無關
...
### ET_SURVEY_QUESTION_TYPE（2026-08-28 新增）

| 代碼 | 顯示名稱 | 說明 |
|------|---------|------|
| SINGLE | 單選 | 教師自訂選項（至少 2 個），學員選一個 |
| TEXT | 問答 | 教師只填題幹（無選項），學員以文字作答（至多 150 字）|

> 與 `ET_QUESTION_TYPE`（測驗題型）**刻意不共用**：問卷沒有多選、測驗沒有問答，
> 值域不同；共用會讓任一邊擴充時污染另一邊。
