...
- [ ] T072 [US5] 實作關閉唯讀處理（2026-07-02 改寫）：課程已關閉時學員仍可開啟並**唯讀回看**已學內容；禁止累積進度 / 作答 / 解鎖 / 填問卷；起始時間未到之課程不可進入（**#363 註**：卡片已改為顯示但不可點擊；⚠️ 後端 `learning` 的 `ensure_can_access` 只判「在籍 OR 擁有者」、**不看 `OPEN_START_AT`**，故「不可進入」目前僅由前端不給入口達成，深連結未被擋——待裁示是否補後端守門）
