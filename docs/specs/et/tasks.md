...
- [ ] T072 [US5] 實作關閉唯讀處理（2026-07-02 改寫）：課程已關閉時學員仍可開啟並**唯讀回看**已學內容；禁止累積進度 / 作答 / 解鎖 / 填問卷；起始時間未到之課程不可進入（卡片於 #363 改為顯示但不可點擊；**後端守門於 #374 補上**——`learning` 403 `ET_LEARN_005`、`progress` 409 `ET_PROGRESS_003`、`attempt` 409 `ET_ATTEMPT_007`，皆豁免擁有者。在 #374 之前「不可進入」僅由前端不給入口達成，深連結未被擋，學員可提前學習並完課）
...
- [x] T162 [US17] 實作 ET10 核可查詢：教師 / 管理者依姓名查全部學員核可課程（含通過 / 不通過 / 撤銷紀錄）；學員自查僅「已通過（有效未撤銷）」；後端依登入身分控管查詢範圍（拒絕學員查他人 / 不通過 / 撤銷）
