...
5. **Given** 週報產生，**When** 渲染內容，**Then** email 內文摘要含每門課程：平均看課進度%（含與上週快照比較之增減）、人數分布（未開始 / 進行中 / 已完課）、完課率、距訖止天數（**不含任何學員姓名**）；並提供**逐學員明細 CSV 下載連結**（點擊後於 ET 系統下載；欄位：姓名、Email、進度%、完課狀態、最後活動時間）
...
- **FR-ET-US14-04**: 系統 MUST 於快照寫入後寄送週報：每位教師收到一封（僅含自己建立之開放中課程）、每位管理者收到一封全域週報；內文 MUST 含各課程平均看課進度%（含與上週快照比較之增減）、人數分布、完課率與距訖止天數，且 **MUST NOT 含任何學員姓名或 Email**（2026-09-15 變更：信件內文會隨轉寄離開所有存取控制，逐學員資料一律由下載端點提供）；並提供逐學員明細之 **CSV 下載連結**（`{{REPORT_CSV_URL}}`，非郵件附件——平台唯一發信服務不支援附件）；當該課程無上週快照時「與上週比較」MUST 顯示「—」而非錯誤
...
- **FR-ET-US14-11**: 系統 MUST 提供週報之逐學員明細 CSV **下載端點**，並以下列規則把關：(1) 連結指向 ET 系統內端點、**需登入**（平台 DP JWT）方可存取，未登入時 MUST 導向登入頁；(2) 授權由 ET 判定——**教師僅可下載自己為擁有者之課程**、**管理者可下載全域**，越權存取 MUST 回應無權限；(3) CSV 內容於**點擊當下即時查詢產生**（非寄信當下之凍結檔），故可能與信中摘要之數字有時間差，週報內文 MUST 註明此點；(4) 連結**不另設有效期**（授權由登入與角色把關，非連結本身），課程關閉後仍可下載歷史明細。
