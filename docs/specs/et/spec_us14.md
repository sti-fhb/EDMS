...
5. **Given** 週報產生，**When** 渲染內容，**Then** email 內文摘要含每門課程：平均看課進度%（含與上週快照比較之增減）、人數分布（未開始 / 進行中 / 已完課）、完課率、距訖止天數、未開始名單；並提供**逐學員明細 CSV 下載連結**（點擊後於 ET 系統下載；欄位：姓名、Email、進度%、完課狀態、最後活動時間）
