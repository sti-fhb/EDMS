...
本模組**獨立於主系統 TBMS 部署**，與 DM 採 SSO（Single Sign-On）共用 user table（USER_ID / 帳號 / 密碼 / 姓名），但兩模組各自管理自己的角色與受訓單位標籤（標籤不與 DM 共用）；與主系統各業務模組之帳號完全切開、不依賴 DP 模組之參數機制（影片格式 / 大小上限 / 排程時間等系統參數由 ET 內部 `ET_PARAM` 自行維護）。
...
| 方向 | 介接對象 | 編碼 | 介接方式 | 說明 |
|------|---------|------|---------|------|
| ET ↔ DM | 文件管理模組（DM） | — | **共用 user table** | USER_ID / 帳號 / 密碼 / 姓名共用；但 ET / DM **各自管理自己的角色與受訓單位標籤**，互不影響 |
