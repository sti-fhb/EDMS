...
- **ET 可維護範本**（**7 類**，`MODULE=ET`；2026-07-17 增列核可通過通知）：儲存於平台 `DP_NOTIFY_TEMPLATE`，由 ET 管理者於 [spec_us15.md](../spec_us15.md) US15「通知範本」分頁維護主旨與內文，並可**啟用 / 停用**（`IS_ACTIVE`）。**教師不可逐課編輯**信件主旨與內文（統一範本）。
...
### ET 可維護範本（7 類；平台 `DP_NOTIFY_TEMPLATE`，`MODULE=ET`，管理者於 US15 維護、可停用）
...
3. ET 後端呼叫平台唯一發信服務，傳 template_code（如 COURSE_INVITE）+ MODULE=ET + 變數 + 收件者
   - ET 可維護 7 類：平台讀取 DP_NOTIFY_TEMPLATE（MODULE=ET, TEMPLATE_CODE）；ET 於呼叫前檢查 IS_ACTIVE，false 則略過寄信（事件照常）
...
> 下列 SMTP 配置與寄件者地址集中由平台發信引擎持有與調校（重試 / 速率 / 重試間隔＝平台級 `MAIL` 參數，見平台 DP；失敗率告警由 IT 監控負責）；**ET 不直接連 SMTP、不持有下列配置**（2026-07-08 集中化）。
...
| `{{REPORT_CSV_URL}}` | 逐學員明細 CSV **下載連結**（指向 ET 系統端點；需登入、教師限自己課程 / 管理者全域；內容即時產生）|

> **2026-08-19 變更（附件 → 下載連結）**：逐學員明細（姓名、Email、進度%、完課狀態、最後活動時間）原設計為**郵件附件**，因**平台唯一發信服務不支援附件**（`DP_EMAIL_LOG` outbox 無附件欄位、`app/dp/notify/` 亦無附件實作），且 ET 不得自建寄件佇列，改以內文變數 `{{REPORT_CSV_URL}}` 提供 **CSV 下載連結**。
>
> **本平台發信服務一律不支援附件**——ET 各類通知信如需傳遞檔案，一律採「系統內下載連結」模式（需登入 + 模組端授權），不得改走自建寄信管道。

#### APPROVAL_PASSED（核可通過通知；US16）

| 變數 | 說明 |
|------|------|
| `{{USER_NAME}}` | 學員姓名 |
| `{{COURSE_NAME}}` | 課程名稱 |
| `{{APPROVED_BY_NAME}}` | 核可人（教師 / 管理者）姓名 |
| `{{APPROVED_AT}}` | 核可時間 |

> 僅核可結果為「通過」時觸發；「不通過」與「撤銷」不寄信（per [spec_us16.md](../spec_us16.md) FR-ET-US16-08）。
...
- **停用不寄**：ET 可維護 7 類若 `IS_ACTIVE = false` 則不寄該信、觸發事件照常；平台系統信 2 類不可停用。
...
- 依賴平台 `DP_NOTIFY_TEMPLATE` 表（由平台 DP 建立）與 ET 7 類範本 `MODULE=ET` seed（[tasks.md](../tasks.md) T129）、平台唯一發信服務與 `DP_EMAIL_LOG` outbox 先行就緒；US15 維護頁（T151）非阻擋——seed 內容即可支撐所有寄信。
...
| 2026-07-17 | 0.4 | 線下核可（US16）：ET 可維護範本由 6 類增為 7 類，新增核可通過通知 `APPROVAL_PASSED`（含變數區塊）；Email 類型總數 8 → 9；停用 / 觸發規則沿用 |
