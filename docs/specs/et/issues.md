...
| ~~12~~ | ~~ET03 待加入邀請追蹤~~ **已廢止（2026-09-18 #362）** | US12 / UCET006 | — | ~~T107 ~ T111~~ | #8 |
...
11. 刪除章節 / 題目 → 本體軟刪除；學員 ET_PROGRESS / ET_QUIZ_ATTEMPT_D **連帶軟刪除**
...
4. ~~**起始時間未到之課程不顯示**~~（**#363 推翻**：改為顯示、卡片不可點擊並標開放時點——原行為與「起始時間未到仍可加入」相乘＝加得進去但看不到，且統計連「已加入課程」都算 0）；**已關閉課程顯示「已關閉」標示，點擊可唯讀回看**（2026-07-02 新增）
...
**涵蓋 Tasks**：
- ~~T086 ET_INVITATION Repository（含狀態流轉）~~ 已於 #362 移除（表已 DROP）
...
5. Email 邀請：教師輸入多筆 Email → 統一範本信件**預覽（不可編輯主旨 / 內文）** → 寄出即加入課程；通知信失敗時該收件人仍已加入，系統列出是哪幾筆（2026-09-18 #362 變更，原為寫 status_code、待加入清單可再寄）
...
## ~~Issue #12：ET03 待加入邀請追蹤~~ — **已於 2026-09-18（#362）廢止**

本 issue 曾於 #342 部分落地（待加入清單 / 再次寄送 / 撤回），#362 將其連同
`ET_INVITATION` 整組移除。**不是延後，是取消**——邀請即加入之後沒有「待加入」可追蹤。

取消理由、被放棄的能力與「若日後要邀請未註冊外部 Email 須重新檢討」的條件，見
[spec_us12.md](spec_us12.md)。T107~T111 編號不重用。
...
3. 教師收到自己開放中課程之週報、管理者收到全域週報；內文含平均進度%（與上週比較）、人數分布、完課率、距訖止天數，以及逐學員明細 **CSV 下載連結**（**內文不列任何學員姓名**，誰未開始由 CSV 回答）（`{{REPORT_CSV_URL}}`；**非郵件附件**——平台唯一發信服務不支援附件）
...
- ~~Issue #11 (US11) / #12 (US12) 完成 #8 後可平行~~（#12 已廢止，#362）
