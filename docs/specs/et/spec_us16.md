...
- **FR-ET-US16-03**: 系統 MUST 以「線上完課」為核可前提；未完課學員 MUST NOT 顯示核可按鈕（不提供核可入口，狀態顯示「未達核可資格」）；批次核可時對名單中未完課者 MUST 跳過並提示

    > 🔴 **完課狀態 MUST 以即時計算取得，MUST NOT 讀 `ET_ENROLLMENT.COMPLETION_STATUS`**（2026-09-17 修訂）。本條原文寫「（`ET_ENROLLMENT.COMPLETION_STATUS = COMPLETED`）」，但該欄位全庫**只在加入課程時寫入一次 `NOT_STARTED`、沒有任何路徑推進它**（`COMPLETED_AT` 更連寫入點都沒有）。照原文實作，核可按鈕會對每一位學員都不出現，**而且不會報錯**。US9（#322）與 ET04（#284）已分別踩過同一個坑並改為即時導出。
    >
    > 🔴 **判定 MUST 以「完成項目數 / 總項目數」比較，MUST NOT 用完課率百分比**：`completion_pct` 會四捨五入，201 個項目完成 200 個時回 100，用它判定會讓最後一項尚未完成就開放核可，且無處察覺。實作一律走 `enrollment/rules.is_course_completed(done=, total=)`。
...
- **FR-ET-US16-07**: 核可 / 撤銷之執行者 MUST 限該課程 owner（教師）或管理者；非 owner 之其他教師 MUST NOT 顯示核可操作
    ...
    > ⚠️ **管理者的部分於 ET-18 僅後端成立**（SA 裁示 2026-09-17 Q1 = C）。後端 ET03 的三支讀取端點、兩支 CSV 匯出與核可 / 撤銷端點皆已放寬為 owner ∪ 管理者；但**前端 ET03 的課程下拉仍為 `scope=mine`**，管理者在畫面上選不到他人課程，故實際進入路徑待 **US17（ET-19）**補齊。這是刻意的中間狀態，不是漏做。
    >
    > **未放寬**：重置重考次數與移除學員（US9 的課程管理動作）維持 owner-only，不在本 US 的裁示範圍。
...
- **FR-ET-US16-11**: ET_APPROVAL MUST 以 (COURSE_ID, USER_ID) 邏輯唯一（0～1 筆 / 學員 / 課程）；寫入 MUST 防並發覆寫

    > **本條的「以 VERSION 樂觀鎖檢核」放寬為「防並發覆寫」**（SA 裁示 2026-09-17）。核可的對象多半是尚無 `ET_APPROVAL` 列的「待核可」學員——**沒有列就沒有版本號可帶**，要求前端逐人給版本號會讓批次端點無法使用。
    >
    > 因此實作分兩種守門，效果等價：**撤銷**（單筆、會作廢既有結果）以 `VERSION` 檢核，不符回 409；**核可**（可批次、非破壞性）以條件式 `WHERE` 承擔——重核認 `IS_REVOKED = true`、首次核可以唯一鍵 `ON CONFLICT DO NOTHING`，衝突一律表現為「已跳過」而非靜默覆寫。`VERSION` 欄位仍存在且每次寫入遞增。
...
| ET-MSG-ET03-309 | 提示 | 部分學員已有核可紀錄，已跳過（N 筆）| 批次含**已核可且未撤銷**者（2026-09-17 SA 裁示新增）|
| ET-MSG-ET03-310 | 提示 | 部分學員已不在此課程，已跳過（N 筆）| 批次含**已被移除**者（2026-09-17 新增）|

> **`309` 為何需要**（SA 裁示 Q2 = A）：wireframe 的單列 UI 對已有核可結果者**只給「撤銷」**，刻意不讓人直接改判（改判須先撤銷並填原因，見 `FR-ET-US16-06`）。但已通過那列的勾選框並未停用，若批次不加這道跳過，「批次不通過」會把「已通過」翻成「未通過」且 `REVOKE_REASON` 是空的——整條原因必填被繞過，稽核上看到結果被改掉卻查不到理由。
>
> **`310` 為何需要**：完課判定數的是 `ET_PROGRESS` 的列，**不看在籍**；而移除學員走 `IS_REMOVED`、學習歷史刻意保留。所以「曾完課、之後被移除」的學員完課條件仍成立，少了這道跳過會替不在班上的人建立核可列，而他在 US17 核可查詢裡查得到。正常 UI 走不到（清單不列已移除者），這是防繞過與「載入後才被移除」的競態。
>
> **`303` / `309` / `310` 三者不可合併成一句「已跳過 N 筆」**：教師的下一步不同——等他完課、先撤銷、重新邀請。
>
> **`304` 與 `303` 的分流由前端依筆數決定**：後端讓單筆與批次走同一支端點（單筆即名單長度為 1），未完課一律回逐筆理由；前端單筆顯示 `304`（錯誤）、批次顯示 `303`（提示）。後端因此**不註冊對應的錯誤碼**——那會是一個沒有任何地方 raise 的碼。
